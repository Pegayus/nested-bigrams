...
def _repl(st):
    '''
    Removes the variable names in the AST and replaces them with 'VAR'
    :param st: a string, the result of ast.dump on a node
    :return: the same string as input with variable names replaced with 'VAR'
    '''
    allvar = re.findall('\(name=(.*?),', st)
    allvar.extend(re.findall(', name=(.*?),', st))
    allvar.extend(re.findall('\(id=(.*?),', st))
    allvar.extend(re.findall('\(arg=(.*?),', st))
    allvar.extend(re.findall(', asname=(.*?)\)', st))
    allvar.extend(re.findall(', attr=(.*?),', st))
    allvar.extend(re.findall('ImportFrom\(module=(.*?),', st))
    allvar = list(set(list(allvar)))
    for i in allvar:
        st = st.replace(i, 'VAR')
    return st
...
def code_bigram(node):
    '''
    This version uses the _repl and gives nested bigrams independent from the variable names
    :param node: an ast node
    :return: bigrams of the code in a list
    '''
    result = []
    if node.__class__ == ast.Module:
        ...
        for child in ast.iter_child_nodes(node):
            first = _repl(ast.dump(node))
            second = _repl(ast.dump(child))
            bigram= [first, second]
...
def code_bigram2(node):
    '''This version is old and does not make nested bigrams independent from variables.
        Use the one above for that purpose.'''
    result = []
    if node.__class__ == ast.Module:
        for child in ast.iter_child_nodes(node):
            bigram_collector = code_bigram(child)
